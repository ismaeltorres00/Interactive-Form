...
function Row({
  ...
  return (
    <tr
      className={`border-b border-zinc-50 last:border-0 transition dark:border-zinc-800 ${
        muted ? 'opacity-60' : 'hover:bg-zinc-50 dark:hover:bg-zinc-800/50'
      }`}
    >
      <td className="px-5 py-4">
        <div className={`font-medium ${muted ? 'text-zinc-500 dark:text-zinc-400' : 'text-zinc-800 dark:text-zinc-100'}`}>
          {s.client_name}
        </div>
        {s.client_email && (
          <div className="text-xs text-zinc-400 dark:text-zinc-500">{s.client_email}</div>
        )}
      </td>
      ...
      <td className="px-5 py-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-zinc-400 dark:text-zinc-500">{s.progress}%</span>
        </div>
      </td>

      <td className="px-5 py-4 text-xs text-zinc-400 dark:text-zinc-500">
      ...
      <td className="px-5 py-4">
        <div className="flex items-center gap-3">
          {!muted && (
            <>
              <Link href={`/sessions/${s.id}`} className="text-xs font-medium text-violet-600 hover:underline dark:text-violet-400">
                Ver respuestas
              </Link>
              <a
                href={`/form/${s.id}`}
          ...
          {muted && onRestore && (
            <button
              onClick={onRestore}
              className="text-xs font-medium text-zinc-500 hover:text-violet-600 transition dark:text-zinc-400 dark:hover:text-violet-400"
            >
              Restaurar
            </button>
          )}
          ...
          {onArchive && (
            <button
              onClick={onArchive}
              title="Archivar"
              className="rounded p-1 text-zinc-300 hover:bg-zinc-100 hover:text-zinc-500 transition dark:text-zinc-600 dark:hover:bg-zinc-800 dark:hover:text-zinc-400"
            >
          ...
          {confirmDelete ? (
            <div className="flex items-center gap-1.5">
              <span className="text-xs text-zinc-500 dark:text-zinc-400">¿Eliminar?</span>
              <button
              ...
              <button
                ...
                className="rounded border border-zinc-200 px-2 py-0.5 text-xs text-zinc-500 hover:bg-zinc-50 transition dark:border-zinc-700 dark:text-zinc-400 dark:hover:bg-zinc-800"
              >
                No
              </button>
            </div>
            ...
            <button
              ...
              title="Eliminar"
              className="rounded p-1 text-zinc-300 hover:bg-red-50 hover:text-red-400 transition dark:text-zinc-600 dark:hover:bg-red-900/20 dark:hover:text-red-400"
            >
              <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8}
...
export function ClientTable({ active: initialActive, archived: initialArchived }: Props) {
  ...
  const tableHead = (
    <thead>
      <tr className="border-b border-zinc-100 bg-zinc-50 text-left text-xs font-medium uppercase tracking-wide text-zinc-400 dark:border-zinc-800 dark:bg-zinc-800/50 dark:text-zinc-500">
      ...
      {(archivedRows.length > 0 || archiveOpen) && (
        <div className="overflow-hidden rounded-xl border border-zinc-100 bg-white dark:border-zinc-800 dark:bg-zinc-900">
          <button
            ...
            className="flex w-full items-center justify-between px-5 py-3 text-left hover:bg-zinc-50 transition dark:hover:bg-zinc-800"
          >
            ...
            <svg
              className={`h-4 w-4 text-zinc-300 transition-transform duration-200 dark:text-zinc-600 ${archiveOpen ? 'rotate-180' : ''}`}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          {archiveOpen && archivedRows.length === 0 && (
            <p className="border-t border-zinc-50 px-5 py-4 text-sm text-zinc-300 dark:border-zinc-800 dark:text-zinc-600">
