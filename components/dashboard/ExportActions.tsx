    ...
    <div className="flex items-center gap-2">
      ...
      {driveUrl && (
        <a
          ...
          className="flex items-center gap-1.5 rounded-lg border border-zinc-200 bg-white px-3 py-1.5 text-xs font-medium text-zinc-600 hover:bg-zinc-50 transition"
      ...
      <div className="relative" ref={ref}>
        ...
        {open && !emailOpen && (
          <div className="absolute right-0 top-full z-20 mt-1.5 w-56 overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-lg">
            {/* PDF */}
            <button onClick={handlePdf} className="flex w-full items-center gap-3 px-4 py-2.5 text-sm text-zinc-700 hover:bg-zinc-50 transition">
            ...
            {/* Word */}
            <button onClick={handleWord} disabled={wordLoading} className="flex w-full items-center gap-3 px-4 py-2.5 text-sm text-zinc-700 hover:bg-zinc-50 transition disabled:opacity-50">
            ...
            {/* Drive */}
            {driveConnected ? (
              <button onClick={handleDrive} disabled={driveLoading} className="flex w-full items-center gap-3 px-4 py-2.5 text-sm text-zinc-700 hover:bg-zinc-50 transition disabled:opacity-50">
              ...
              <a
                ...
                className="flex w-full items-center gap-3 px-4 py-2.5 text-sm text-zinc-400 hover:bg-zinc-50 transition"
              >
                <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none">
            ...
            {/* Email */}
            <button onClick={() => { setEmailOpen(true); setOpen(false) }} className="flex w-full items-center gap-3 px-4 py-2.5 text-sm text-zinc-700 hover:bg-zinc-50 transition">
              <svg className="h-4 w-4 text-zinc-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8}
            ...
            <a href={whatsappHref} target="_blank" rel="noopener noreferrer" onClick={() => setOpen(false)}
              className="flex w-full items-center gap-3 px-4 py-2.5 text-sm text-zinc-700 hover:bg-zinc-50 transition">
            ...
            {/* Copy link */}
            <button onClick={handleCopy} className="flex w-full items-center gap-3 px-4 py-2.5 text-sm text-zinc-700 hover:bg-zinc-50 transition">
              <svg className="h-4 w-4 text-zinc-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8}
        ...
        {/* Email panel */}
        {emailOpen && (
          <div className="absolute right-0 top-full z-20 mt-1.5 w-80 rounded-xl border border-zinc-200 bg-white p-4 shadow-lg">
            <div className="mb-3 flex items-center justify-between">
              <p className="text-sm font-semibold text-zinc-800">Enviar por email</p>
              <button onClick={() => setEmailOpen(false)} className="text-zinc-300 hover:text-zinc-500 transition">
                <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                </svg>
              </button>
            </div>
            <div className="mb-3 space-y-1.5 rounded-lg bg-zinc-50 px-3 py-2.5 text-xs text-zinc-500">
              <p><span className="font-medium text-zinc-600">Para:</span> {clientEmail ?? 'sin email registrado'}</p>
              <p><span className="font-medium text-zinc-600">Asunto:</span> Resumen del formulario de branding{companyName ? ` · ${companyName}` : ''}</p>
            </div>
            <p className="mb-3 text-xs text-zinc-400">Se abrirá tu cliente de correo con el mensaje pre-rellenado.</p>
            <div className="flex gap-2">
              ...
              <button onClick={() => setEmailOpen(false)}
                className="rounded-lg border border-zinc-200 px-3 py-2 text-xs font-medium text-zinc-500 hover:bg-zinc-50 transition">
                Cancelar
              </button>
            </div>
          </div>
